        ...
        <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 my-12 '>
            {
                exclusiveOffers.map((items)=>{
                    return(
                        <div key={items._id} className='group relative flex flex-col items-start justify-between gap-1 pt-12 md:pt-18 px-4 rounded-xl text-white 
                        bg-no-repeat bg-cover bg-center' style={{backgroundImage:`url(${items.image})`}}>
                            ...
                            <div>
                                ...
                                <p className='text-xs text-white/70 mt-3'>{items.description}</p>
                            ...
                            <button className=' flex items-center gap-2 font-medium cursor-pointer mt-4 mb-5'>View Offers
                                <img src={assets.arrowIcon}  className="invert group-hover:translate-x-1 transition-all"alt="arrowicon"/>
                            </button>
                        </div>
                    )
                })
